...
Web questions cascade through real search providers (server-side, in order):

1. **Groq built-in** (`groq/compound` → `groq/compound-mini`) — if the account has access
2. **LangSearch** (`LANGSEARCH_API_KEY` — api.langsearch.com, free tier) — real Bing-style web results
3. **Brave Search API** (`BRAVE_API_KEY`) — real-time news/scores/prices
4. **Wikipedia** — zero-config fallback

Answers are generated by Llama with **strict grounding** — only facts present in the retrieved results may be stated, and every source shown is a real URL from an actual search response. Provider used is shown in the badge (🌐 Web · LangSearch / Brave / Wikipedia). Add the keys in Vercel → Environment Variables → Redeploy.
